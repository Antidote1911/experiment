...
fn encrypt_file<P: AsRef<Path>>(input_path: P, output_path: P, key: &[u8]) -> Result<(), anyhow::Error>  {
    ...
    let mut salt: [u8; SALT_SIZE] = rand::rng().random();
    let mut nonce: [u8; NONCE_SIZE] = rand::rng().random();

    let mut derivedKey = [0u8; 32]; // Can be any desired size
    Argon2::default().hash_password_into(key, &salt, &mut derivedKey).unwrap();

    //let nonce= XNonce::from_slice(NONCE);

    // create the stream encryptor
    let cipher = XChaCha20Poly1305::new((&derivedKey).into());
